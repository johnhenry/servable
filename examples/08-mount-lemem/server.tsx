/**
 * Serving a `lemem`-packaged directory through servable -- no new servable
 * primitive needed. `lemem`'s `createRouter()` already produces a
 ...
 * and map to routes), a `lemem` archive is already a live, Fetch-shaped
 ...
 * lemem's own file map knows nothing about. `alias: { "": "index.html" }`
 * (lemem's path-after-alias is always leading-slash-stripped, so this is
 ...
 *
 * Run with:
 *   npm run build && node dist/examples/08-mount-lemem/server.js
 * Then:
...
// build && node dist/examples/08-mount-lemem/server.js` is always run from,
// per the doc comment above), not the compiled file's own location.
const files = await fromDirectory(join(process.cwd(), "examples/08-mount-lemem/site"));
